...
class Mp3:
    ...
    def _store_media_file(self, file_stats: str, full_file_and_path, file_name):
        ...
        found_song = self._db.execute(f"select * from main.medias where file_name = \"{file_name}\"").fetchall()

        if len(found_song) > 0:
            self._db.execute(f"INSERT INTO main.medias (title, file_name, size, media_type) VALUES (\"{file_name}\", \"{file_name}\", {file_stats.st_size}, 'audio/mpeg');")
